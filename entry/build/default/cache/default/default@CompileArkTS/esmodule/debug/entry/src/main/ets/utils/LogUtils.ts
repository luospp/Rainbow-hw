export class LogUtil {
     ...
     * 解析调用栈信息
     */
    private static parseCallerInfo(stack: string): {
        className: string;
        lineNumber: string;
    } {
        // 默认值
     ...
     * 格式化消息
     */
    private static formatMessage(callerInfo: {
        className: string;
        lineNumber: string;
        ...
        if (args && args.length > 0) {
            try {
                ...
                formattedMessage = `${message} ${args.map(arg => typeof arg === 'object' ? JSON.stringify(arg) : arg).join(' ')}`;
            }
            catch (error) {
